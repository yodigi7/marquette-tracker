...
describe("backup snapshot and restore", () => {
  ...
  it("replaces records and settings, resets sync flags, and re-derives cycle structure", async () => {
    ...
    incoming.dayRecords = incoming.dayRecords.map((record) => ({
      ...record,
      cycleId: "stale-cycle",
      dayInCycle: 99,
    }));
    ...
    expect(result).toMatchObject({
      cycleCount: 2,
      dayRecordCount: prepared.summary.dayRecordCount,
    });
  ...
  it("removes autogenerated rows a restored older document reintroduces", async () => {
    ...
    const legacyPrepared = {
      ...
      document: {
        ...prepared.document,
        data: { ...prepared.document.data, dayRecords: legacy.data.dayRecords },
      },
    };
  ...
  it("rejects duplicate day-record dates before replacing the dataset", async () => {
    ...
    await expect(state(store).restoreBackup(prepared)).rejects.toMatchObject({
      code: "duplicate-date",
    });
