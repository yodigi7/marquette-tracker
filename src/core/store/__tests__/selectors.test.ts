...
describe("projectedCyclesThrough", () => {
  it("returns nothing when the setting is off", () => {
    ...
    expect(
      projectedCyclesThrough(output, settings({ projectFutureCycles: false }), TODAY, "2027-01-31"),
    ).toEqual([]);
  });

  it("returns nothing when there is no engine output", () => {
    expect(
      projectedCyclesThrough(null, settings({ projectFutureCycles: true }), TODAY, "2027-01-31"),
    ).toEqual([]);
  });

  it("returns projected cycles when the setting is on", () => {
    const output = outputFor();
    const projected = projectedCyclesThrough(
      output,
      settings({ projectFutureCycles: true }),
      TODAY,
      "2027-01-31",
    );
  ...
  it("bounds the returned range by the requested date", () => {
    const output = outputFor();
    const near = projectedCyclesThrough(
      output,
      settings({ projectFutureCycles: true }),
      TODAY,
      "2026-10-15",
    );
    const far = projectedCyclesThrough(
      output,
      settings({ projectFutureCycles: true }),
      TODAY,
      "2028-01-01",
