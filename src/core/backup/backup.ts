...
function migrate(input: unknown): unknown {
  ...
  for (let fromVersion = version; fromVersion < CURRENT_BACKUP_VERSION; fromVersion += 1) {
    ...
    const settings = isRecord(data.settings) ? { ...DEFAULT_SETTINGS, ...data.settings } : data.settings
    current = {
      ...current,
...
function normalizeDocument(input: unknown): BackupDocument {
  ...
  if (typeof input.appVersion !== 'string' || !isIsoDate(input.exportedAt) || !isRecord(input.data)) {
...
function validateMeta(meta: { version: number; synced: boolean; createdAt: string; updatedAt: string }, code: BackupErrorCode): void {
...
function validateCycle(cycle: CycleEntity): void {
  if (!cycle.id || !isDateKeyValue(cycle.day1) || !Number.isInteger(cycle.cycleNo) || cycle.cycleNo < 1) {
  ...
  if (typeof cycle.notes !== 'string' || (cycle.pinned !== undefined && typeof cycle.pinned !== 'boolean')) {
...
function validateDayRecord(record: DayRecordEntity, today: string): void {
  if (!record.id || typeof record.cycleId !== 'string' || !isDateKeyValue(record.date) || record.date > today) {
    fail(record.date > today ? 'future-date' : 'invalid-record', 'Day record identity or date is invalid')
  }
  ...
  if (record.bbt !== undefined && record.bbt !== null && (typeof record.bbt !== 'number' || !Number.isFinite(record.bbt))) {
    fail('invalid-record', 'BBT value is invalid')
  }
  if (record.symptoms !== undefined && (!Array.isArray(record.symptoms) || record.symptoms.some((item) => typeof item !== 'string'))) {
    fail('invalid-record', 'Symptoms are invalid')
  }
  if (record.pregnancyTest !== undefined && !['negative', 'positive'].includes(record.pregnancyTest)) {
...
function validateSettings(settings: SettingsEntity): void {
  ...
  if (!Number.isInteger(settings.postPeakDays) || settings.postPeakDays < 0 || settings.postPeakDays > 10) {
    fail('invalid-settings', 'Post-Peak days must be an integer between 0 and 10')
  }
  if (!Number.isInteger(settings.historyWindow) || settings.historyWindow < 1 || settings.historyWindow > 12) {
    fail('invalid-settings', 'History window must be an integer between 1 and 12')
  }
  if (!['light', 'dark', 'system'].includes(settings.theme) || !['monday', 'sunday'].includes(settings.weekStart)) {
...
export function createBackup(snapshot: BackupSnapshot, options: BackupOptions = {}): BackupDocument {
  return {
    format: BACKUP_FORMAT,
    formatVersion: CURRENT_BACKUP_VERSION,
...
export function prepareBackupDocument(input: unknown, options: PrepareOptions = {}): PreparedBackup {
