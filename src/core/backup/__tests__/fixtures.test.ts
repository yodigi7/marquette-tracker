...
describe("backup round-trip fixtures", () => {
  it("round-trips user records and cycle metadata", () => {
    ...
    expect(
      prepared.document.data.dayRecords.find((record) => record.id === "user-peak"),
    ).toMatchObject({
      monitor: "peak",
      mucus: "low",
      notes: "User note",
    ...
    expect(
      prepared.document.data.dayRecords.find((record) => record.id === "user-low"),
    ).toMatchObject({
      monitor: "low",
      synced: false,
    });
    expect(
      prepared.document.data.dayRecords.find((record) => record.id === "user-high"),
    ).toMatchObject({
      monitor: "high",
