...
function bandByDay(day: number): HTMLElement | null {
  return (
    screen.getAllByTestId("day-band").find((el) => Number(el.getAttribute("data-day")) === day) ??
    null
  );
}
...
describe("StripChart", () => {
  ...
  it("renders one window treatment with no source attribute", () => {
    ...
    expect(
      document.querySelector(".recharts-reference-area-rect")?.getAttribute("stroke-dasharray"),
    ).toBeNull();
  });
