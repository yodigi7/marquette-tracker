...
describe('buildStripModel', () => {
  ...
  it('spans at least day 1 for an open cycle with no records', () => {
    const model = buildStripModel(cycle({ closedAt: null }), result({ length: null, days: [] }), [], true)
  ...
  it('keeps a null end when the engine reports no end', () => {
    const model = buildStripModel(cycle(), result({ fertileWindow: { begin: 6, end: null, beginRule: 'calendar-day-6', endRule: 'none' } }), [], true)
