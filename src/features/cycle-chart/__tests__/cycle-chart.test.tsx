...
describe("CycleChartView", () => {
  ...
  it("renders a day-band strip and fertile window for a closed cycle (US1)", async () => {
    ...
    expect(document.querySelector(".recharts-reference-area-rect")?.getAttribute("fill")).toBe(
      "var(--fertility-window-fill)",
    );
    expect(
      document.querySelector('[data-legend-label="Low"] span')?.getAttribute("class"),
    ).toContain("bg-fertility-monitor-low");
  });
  ...
  it("keeps user records and drops only the window band while the algorithm is off", async () => {
    ...
    await useAppStore
      .getState()
      .addDayRecord(cycle.id, addDays(start, 13), 14, { monitor: "peak" });
    ...
    expect(
      bands()
        .find((band) => band.getAttribute("data-day") === "14")
        ?.getAttribute("data-monitor"),
    ).toBe("peak");
    expect(
      bands()
        .find((band) => band.getAttribute("data-day") === "19")
        ?.getAttribute("data-monitor"),
    ).toBeUndefined();
