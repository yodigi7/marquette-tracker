...
function LoggingOnlyCard() {
  return (
    <Card>
      ...
      <CardContent>
        <p
          data-testid="history-logging-only"
          className={cn("text-sm", FERTILITY_TEXT_VISUALS.muted)}
        >
          Algorithm is off — logged cycle information remains available, but fertility forecasts and
          derived summaries are hidden.
        </p>
      </CardContent>
    </Card>
  );
      ...
      <Card>
        ...
        <CardContent>
          <p className={cn("text-sm", FERTILITY_TEXT_VISUALS.muted)}>
            Not enough data yet. Complete a cycle to see predictions.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          Forecast
          <Badge variant="outline" className={FERTILITY_FORECAST_VISUAL.text}>
            predicted
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Stat
            ...
            value={`${forecast.nextFertileWindow.begin} → ${forecast.nextFertileWindow.end}`}
          />
        </div>
        <Stat
          label="Based on"
          value={`${forecast.basedOnCycles} cycle${forecast.basedOnCycles === 1 ? "" : "s"}`}
        />
        <p className={cn("text-xs", FERTILITY_TEXT_VISUALS.muted)}>
          ...
          atypical cycle does not shift every date. The averages above are shown for reference and
          are not what produces them.
        </p>
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <Stat
            label="Range"
            value={forecast ? `${forecast.earliestLength}–${forecast.latestLength}` : "—"}
          />
          <Stat
          ...
          <Stat
            label="Fertile days"
            value={`${total} (avg ${fertileTotals.length ? (total / fertileTotals.length).toFixed(1) : 0})`}
          />
        </div>
      </CardContent>
    </Card>
