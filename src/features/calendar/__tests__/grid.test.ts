...
describe('resolveCell', () => {
  ...
  it('shades only recorded days and flags day 1 as menses', () => {
    ...
    expect(fertile.info?.status).toBe('fertile')
    expect(fertile.info?.source).toBe('predicted')

    const day1 = resolveCell([CYCLE], recorded, results, undefined, '2026-08-03', today)
    expect(day1.menses).toBe(true)
  })

  it('keeps unrecorded days blank even inside the predicted fertile band', () => {
  ...
  it('shows the forecast ring only inside the forecast window', () => {
    ...
    const inside = resolveCell([], NO_RECORDS, new Map(), forecast, '2026-09-07', today)
    expect(inside.info).toBeNull()
    expect(inside.forecast).toBe(true)

    const outside = resolveCell([], NO_RECORDS, new Map(), forecast, '2026-09-20', today)
  ...
  it('keeps future days blank (no status band) with only the forecast ring', () => {
    ...
    expect(pastOutOfWindow.info).toBeNull()
    ...
    expect(recordedFuture.info?.status).toBe('fertile')
