    ...
    >
      {!cycle || cycleDay === null ? (
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          ...
          {interpreted && phase ? (
            <span
              className={cn("font-medium", FERTILITY_TEXT_VISUALS.body)}
              data-testid="calendar-summary-phase"
            >
              {calendarPhaseLabel(phase)}
            </span>
          ) : (
            <span
              className={cn("font-medium", FERTILITY_TEXT_VISUALS.muted)}
              data-testid="calendar-summary-mode"
            >
              Logging only
            </span>
          )}
