...
Milestone 8 (Settings) complete: goal, algorithm on/off (log-only when off, app-wide), post-Peak days (default 4), selectable inferred post-Peak fill mode with persistent deletion suppression and monitor-High interruption, history window, theme (system/light/dark), calendar week-start (Monday/Sunday), configurable cycle-length protocol band (default 21–42), persisted chart overlay toggles (BBT/mucus/intercourse), and a confirmed clear-all-data danger zone (current full suite: 316 tests, all green).
