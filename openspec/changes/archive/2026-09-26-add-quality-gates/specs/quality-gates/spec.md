...
Defines how the project enforces formatting, linting, type checking, testing, and building before changes
reach the main branch — the committed style contract, the single canonical command that runs every gate,
the local Git hooks that apply them, and the pull-request validation that mirrors them.
